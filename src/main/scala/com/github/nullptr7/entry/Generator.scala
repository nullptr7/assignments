...
object Generator {
  ...
  val countryCode: Vector[String] = Vector(
  ...
  def generateDummyRecords(): Unit = {
    val writer = new BufferedWriter(new FileWriter(new File("src/main/resources/country.csv")))
    for (_ <- 0 until 10000000) {
      val (id, value) = generate
      writer.write(s"""$id,"$value"\n""")
    }
    writer.close()
  }

  def generate: (Int, String) = {
    val id  = Random.nextInt(1000)
    val idx = Random.nextInt(countryCode.length)
    (id, countryCode(idx))
  }
}
